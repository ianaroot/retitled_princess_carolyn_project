...
Replace old editor with new editorV2 in production.
...
// Note: Validators and errors are imported by the modules that use them
...
export async function initEditor(botId, container, svgContainer, editorPanel) {
  ...
  // Wire history to store (circular dependency required for Store.canUndo()/canRedo())
  // See Step 03 for discussion of circular dependency and alternative approaches
  ...
  const nodeRenderer = new NodeRenderer(container, store)
