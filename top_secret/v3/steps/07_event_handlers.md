...
- Step 03: History
- Step 05: Sync Layer (SyncManager)
- Step 06: Rendering Layer (NodeRenderer, ConnectionRenderer)
...
### Handlers Call SyncManager, Never History Directly

**CRITICAL:** Event handlers never call `history.push()` directly. Only SyncManager calls `history.push()` after successful server sync.

```javascript
// CORRECT: Handler calls SyncManager, which handles everything
async finishConnection(event, targetClientId) {
  // Handler does NOT call store.addConnection() directly
  // Handler does NOT call history.push() directly
  // SyncManager handles: optimistic update, server sync, history push, rollback
  await this.syncManager.createConnection(sourceId, targetClientId)
}

// Inside SyncManager.createConnection():
async createConnection(sourceId, targetId) {
  const clientId = generateUUID()
  
  // 1. Optimistic: Update store immediately
  this.store.addConnection(new Connection({ clientId, sourceId, targetId }))
  
  try {
    // 2. Sync with server
    const response = await this.api.createConnection(sourceId, targetId, clientId)
...
- Rollback is clean: just restore store, no history manipulation
- Single source of truth for when history is updated

### Drag Has Final Position on MouseUp

Drag updates are continuous, but sync and history happens only on mouseup.

```javascript
onMouseMove(event) {
  // Continuous UI updates during drag (no sync, no history)
  this.store.updateNode(clientId, { position: { x, y } })
}

onMouseUp(event) {
  // Single sync and history push at end
  if (this.hasMoved && node) {
    // SyncManager handles: server sync + history push
    this.syncManager.updateNodePosition(clientId, node.position.x, node.position.y)
      .catch(err => console.error('Failed to sync position:', err))
  }
}
...
**Why:**
- Continuous sync/history pushes would create many undo steps
- User expects single undo to revert entire drag
- Aligns with user mental model
- SyncManager batches the position update
