...
**Important:** History is NOT stored in Store.history. Components receive History explicitly to avoid circular dependencies.

```javascript
// state/History.js

import { MAX_HISTORY, EVENTS } from '../constants.js'
...
class History {
   ...
   * 
   * IMPORTANT: Always pair with endBatch() or use batch() method
   */
  startBatch() {
   ...
   * @param {string} description - Human-readable description
   * @throws {Error} If endBatch called without matching startBatch
   */
  endBatch(description) {
    this.batchDepth--
    
    if (this.batchDepth < 0) {
      // Reset and throw - prevents orphaned batch state
      this.batchDepth = 0
      this.autoPush = true
  ...
  /**
   * Reset batch state (for error recovery)
   */
  resetBatch() {
    this.batchDepth = 0
    this.autoPush = true
...
The History class receives Store in constructor. Components that need History receive it explicitly (no circular dependency).
...
export async function initEditor(botId, container) {
  ...
  // NO: Don't set store.history = history (circular dependency)
  // YES: Pass history explicitly to components that need it
...
History can auto-push on Store changes via subscriber:

```javascript
// index.js - optional auto-push setup
let isUndoing = false
...
store.subscribe((event, data) => {
  ...
  if (event === EVENTS.GRAPH_REPLACE) return
...
describe('History', () => {
  ...
  beforeEach(() => {
    ...
    // Note: store.history is NOT set - no circular dependency
  })
