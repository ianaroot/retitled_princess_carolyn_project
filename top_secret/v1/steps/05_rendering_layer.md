...
// Constants may be used for node dimensions in future enhancements
```
...
class NodeRenderer {
  ...
  async fetchPreview(clientId, element) {
    ...
    const serverId = this.store.history?.syncManager?.clientToServer?.get(clientId) || node.serverId
